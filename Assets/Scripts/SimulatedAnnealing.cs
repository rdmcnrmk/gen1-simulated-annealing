...
public class SimulatedAnnealing
{
    ...
    /// </summary>
    public List<System.Tuple<List<Vector3>, float>> solutions = new List<System.Tuple<List<Vector3>, float>>();
    ...
    public SimulatedAnnealing()
    {
        ...
        for (int i = 0; i < 30; i++)
        {
            ...
            initialWorld.Locations.Add(new Vector3(r * Mathf.Sin(angle), r * Mathf.Cos(angle), 0));
        }
    }
    ...
    public void Findsolution()
    {
        ...
        World currentSolution = new World(new List<Vector3>(initialWorld.Locations));

        // Randomly reorder the tour
        currentSolution.Locations.Shuffle();

        // Set as current best
        World best = new World(new List<Vector3>(currentSolution.Locations));

        // Loop until system has cooled
        while (temp > 1)
        {
            // Create new neighbour tour
            World newSolution = new World(new List<Vector3>(currentSolution.Locations));
            ...
            Vector3 citySwap1 = newSolution.Locations[tourPos1];
            Vector3 citySwap2 = newSolution.Locations[tourPos2];
            ...
            {
                currentSolution = new World(new List<Vector3>(newSolution.Locations));
            }

            // Keep track of the best solution found
            if (currentSolution.GetDistance() < best.GetDistance())
            {
                best = new World(new List<Vector3>(currentSolution.Locations));
            }

            // Cool system
            temp *= 1 - coolingRate;

            if(solutions.Count == 0 || solutions[solutions.Count-1].Item2 > currentSolution.GetDistance())
            {
                Debug.Log(currentSolution.GetDistance());
                solutions.Add(new System.Tuple<List<Vector3>, float>(currentSolution.Locations, currentSolution.GetDistance()));
            }
        }
    }
}